...
def music_play(v_inp):   
    ...
    with os.scandir("../ch6/chat") as files:
        for file in files:
            if (firstname in file.name.lower() or lastname in file.name.lower()) and "mp3" in file.name:
    ...
    if len(mysongs)==0:
        ...
        mixer.init()
        mixer.music.load(f'../ch6/chat/{mysong}')
        mixer.music.play()
